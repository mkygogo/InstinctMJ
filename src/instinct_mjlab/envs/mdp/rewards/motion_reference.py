...
def base_position_imitation_gauss(
    ...
    base_pos = robot.data.root_link_pos_w  # (batch_size, 3)
...
class base_position_offset_imitation_gauss(ManagerTermBase):
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRLEnv):
        ...
        self.asset_position_marker = torch.ones_like(self.asset.data.root_link_pos_w) * torch.nan  # (batch_size, 3)
        self.asset_quat_marker = torch.ones_like(self.asset.data.root_link_quat_w) * torch.nan  # (batch_size, 4)
    ...
    def compute_base_position_offset(self, in_base_frame: bool):
        ...
        asset_pos_offset = self.asset.data.root_link_pos_w - self.asset_position_marker  # (batch_size, 3)
    ...
    def refresh_markers(self):
        ...
        self.asset_position_marker[landmarker_refresh_mask] = self.asset.data.root_link_pos_w[landmarker_refresh_mask]
        self.asset_quat_marker[landmarker_refresh_mask] = self.asset.data.root_link_quat_w[landmarker_refresh_mask]
...
def base_rot_imitation_gauss(
    ...
    quat = asset.data.root_link_quat_w  # (batch_size, 4)
