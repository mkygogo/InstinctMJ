...
if TYPE_CHECKING:
    from instinct_mjlab.envs import ManagerBasedRLEnvCfg
...
def _space_flatdim(space: Any) -> int:
    """Compute flattened dimension for both gym-style and mjlab-style spaces."""
    subspaces = getattr(space, "spaces", None)
    if isinstance(subspaces, dict):
        return int(sum(_space_flatdim(subspace) for subspace in subspaces.values()))

    shape = getattr(space, "shape", None)
    if shape is None:
        raise TypeError(f"Unable to infer flattened dimension from space: {type(space)}")
...
class InstinctRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv):
        ...
        if hasattr(self.unwrapped, "action_manager"):
            ...
            self.num_actions = _space_flatdim(self.unwrapped.single_action_space)
        if hasattr(self.unwrapped, "observation_manager"):
            ...
            self.num_obs = _space_flatdim(self.unwrapped.single_observation_space["policy"])
        ...
        if (
            ...
            # Not checked for DiectRlEnv
            self.num_critic_obs = _space_flatdim(self.unwrapped.single_observation_space["critic"])
        else:
