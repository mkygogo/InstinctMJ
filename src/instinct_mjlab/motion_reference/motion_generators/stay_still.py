...
from instinct_mjlab.motion_reference import MotionReferenceData, MotionReferenceState
from instinct_mjlab.motion_reference.motion_buffer import MotionBuffer
...
class StayStillMotion(MotionBuffer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # (num_envs, num_joints)
        self.rest_joint_pos = torch.zeros_like(self.articulation_view.data.joint_pos)
    ...
    def fill_motion_data(
        ...
        joint_pos = self.articulation_view.data.joint_pos.clone()  # (num_envs, num_dofs)
        joint_pos = joint_pos[env_ids]

        base_pos_w = self.articulation_view.data.root_link_pos_w.clone()[env_ids]
        base_quat_w = self.articulation_view.data.root_link_quat_w.clone()[env_ids]
        base_pose_w = torch.cat((base_pos_w, base_quat_w), dim=-1)
